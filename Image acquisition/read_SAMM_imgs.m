function total_dir = read_SAMM_imgs(data_dir,subject,subset)
    gap1='';
    ...
    for k = 3:fold_le
        if folder_list(k).isdir == 1
            subject_list(fold_count) = str2double(folder_list(k).name);
    ...
    if isempty(subject_found)==1
        ...
        subject_dir = strcat(sprintf('%03d',subject),'\');
        ...
        video_cand = strcat(sprintf('%03d',subject),'_',num2str(subset(1)),'_',num2str(subset(2)));
        video_found = find(strcmp(video_cand,subsetcode_list));
